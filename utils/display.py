...
def getFigures(params, time, pos_all, vel_all, quat_all, omega_all, euler_all, commands, wMotor_all, thrust, torque, sDes_traj, sDes_calc):
    x    = pos_all[:,0]
    y    = pos_all[:,1]
    z    = pos_all[:,2]
    q0   = quat_all[:,0]
    ...
    for ii in range(q0Des.shape[0]):
        ...
        thetaDes[ii] = YPR[1]*rad2deg
        phiDes[ii]   = YPR[2]*rad2deg
    # Create subplots and store ax objects
    fig, axs = plt.subplots(5, 1, figsize=(10, 12))

    axs[0].plot(time, x, time, y, time, z)
    axs[0].plot(time, x_sp, '--', time, y_sp, '--', time, z_sp, '--')
    axs[0].grid(True)
    axs[0].legend(['x','y','z','x_sp','y_sp','z_sp'])
    axs[0].set_xlabel('Time (s)')
    axs[0].set_ylabel('Position (m)')

    axs[1].plot(time, xdot, time, ydot, time, zdot)
    axs[1].plot(time, Vx_sp, '--', time, Vy_sp, '--', time, Vz_sp, '--')
    axs[1].grid(True)
    axs[1].legend(['Vx','Vy','Vz','Vx_sp','Vy_sp','Vz_sp'])
    axs[1].set_xlabel('Time (s)')
    axs[1].set_ylabel('Velocity (m/s)')

    axs[2].plot(time, x_thr_sp, time, y_thr_sp, time, z_thr_sp)
    axs[2].grid(True)
    axs[2].legend(['x_thr_sp','y_thr_sp','z_thr_sp'])
    axs[2].set_xlabel('Time (s)')
    axs[2].set_ylabel('Desired Thrust (N)')

    axs[3].plot(time, phi, time, theta, time, psi)
    axs[3].plot(time, phiDes, '--', time, thetaDes, '--', time, psiDes, '--')
    axs[3].plot(time, yaw_tr, '-.')
    axs[3].grid(True)
    axs[3].legend(['roll','pitch','yaw','roll_sp','pitch_sp','yaw_sp','yaw_tr'])
    axs[3].set_xlabel('Time (s)')
    axs[3].set_ylabel('Euler Angle (°)')

    axs[4].plot(time, p, time, q, time, r)
    axs[4].plot(time, pDes, '--', time, qDes, '--', time, rDes, '--')
    axs[4].grid(True)
    axs[4].legend(['p','q','r','p_sp','q_sp','r_sp'])
    axs[4].set_xlabel('Time (s)')
    axs[4].set_ylabel('Angular Velocity (°/s)')

    # Return the figures as ax objects
    return fig, axs
